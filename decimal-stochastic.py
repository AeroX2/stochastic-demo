...
def test():
  ...
  sdo = [sd1[i] | sd2[i] for i in range(BIT_LENGTH_DECIMAL)]
  if (carry):
  ...
  mult = (math.ceil(sum(sda)/float(BIT_LENGTH_DECIMAL)*float(RANDOM_RANGE))) + (sum(sfa)/float(BIT_LENGTH_FLOAT))
  add = (math.ceil(sum(sdo)/float(BIT_LENGTH_DECIMAL)*float(RANDOM_RANGE))) + (sum(sfo)/float(BIT_LENGTH_FLOAT))
